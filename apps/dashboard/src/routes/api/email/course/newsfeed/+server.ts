...
const sendEmailNotification = async (feedId: string, authorId: string, comment?: string) => {
  ...
  const postLink = `https://${feed.org.siteName}.academy.openagentsfoundation.org/courses/${feed.courseId}?feedId=${feed.id}`;

  if (comment) {
    const emailData = [
      {
        from: `"${feed.org.name} - OpenAgents Academy" <notify@mail.openagentsfoundation.org>`,
      ...
      `,
        replyTo: 'noreply@academy.openagentsfoundation.org'
      }
    ];
